...
const form = ref({
  nif: String(props.cliente.nif ?? ''),
  nome: props.cliente.nome ?? '',
  morada: props.cliente.morada ?? '',
  telefone: String(props.cliente.telefone ?? ''),
  email: props.cliente.email ?? '',
  rgpd: props.cliente.rgpd ?? false,
})

function submit() {
  router.put(route('clientes.update', props.cliente.id), form.value)
}
</script>
